...
if (auth_is_trusted_bot(state, user_host)) {
    ...
    if (encoded_ciphertext && encoded_tag) {
      ...
      if (decoded_data && tag && decoded_len > (SALT_SIZE + GCM_IV_LEN) && tag_len == GCM_TAG_LEN) {
            ...
            if (received_timestamp_str && received_nonce_str && command_part) {
              ...
              if (fabs(difftime(time(NULL), received_time)) <= 60) {
                ...
                if (!nonce_is_reused) {
                  ...
                  if (bot_command && strcasecmp(bot_command, "OPME") == 0 && bot_arg1) {
  ...
  if (is_admin) {
    ...
    if (strcasecmp(command, "die") == 0) {
      ...
      if (c) {
        if (arg2) strncpy(c->key, arg2, MAX_KEY - 1);
        c->is_managed = false;
      }
      ...
      config_write(state, state->startup_password);
      irc_printf(state, "PRIVMSG %s :Bot communication password set and saved.\r\n", nick);
    } else if (strcasecmp(command, "+bot") == 0) {
      if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: +bot <nick*!*user@hostmask.com>\r\n", nick);
        return;
      }
      ...
      char uptime_str[128];
      const char *conn_status = (state->status & S_CONNECTED) ? "CONNECTED" : "DISCONNECTED";
      const char *tls_status = state->is_ssl ? "YES" : "NO";
      const char *server_to_display = (state->actual_server_name[0] != '\0') ? state->actual_server_name : (state->current_server_index > 0 ? state->server_list[state->current_server_index - 1] : "N/A");

      if (state->connection_time > 0 && (state->status & S_CONNECTED)) {
        long uptime_seconds = time(NULL) - state->connection_time;
        snprintf(uptime_str, sizeof(uptime_str), "%ldd %ldh %ldm %lds", uptime_seconds / 86400, (uptime_seconds % 86400) / 3600, (uptime_seconds % 3600) / 60, uptime_seconds % 60);
      } else {
      ...
      snprintf(line_buffer, sizeof(line_buffer), "%-*s | %s", max_width, "--- Servers", "Channels ---");
      irc_printf(state, "PRIVMSG %s :%s\r\n", nick, line_buffer);

      int max_rows = (state->server_count > state->chan_count) ? state->server_count : state->chan_count;
      ...
      for (int i = 0; i < max_rows; i++) {
        ...
        if (i < state->server_count) strncpy(server_part, state->server_list[i], sizeof(server_part) - 1);
        if (current_chan) {
          bool am_i_opped = false;
          for (int j = 0; j < current_chan->roster_count; j++) {
            if (strcasecmp(current_chan->roster[j].nick, state->current_nick) == 0 && current_chan->roster[j].is_op) {
              am_i_opped = true;
              break;
            }
          }
          const char *op_prefix = am_i_opped ? "@" : "";
          const char *status_str = (current_chan->status == C_IN) ? "IN" : "OUT";
          const char *managed_str = current_chan->is_managed ? " [M]" : "";

          if (current_chan->key[0] != '\0') {
            snprintf(chan_part, sizeof(chan_part), "%s%s (Key: %s) (%s)%s", op_prefix, current_chan->name, current_chan->key, status_str, managed_str);
          } else {
            snprintf(chan_part, sizeof(chan_part), "%s%s (%s)%s", op_prefix, current_chan->name, status_str, managed_str);
          }
          current_chan = current_chan->next;
        }
        snprintf(line_buffer, sizeof(line_buffer), "%-*s | %s", max_width, server_part, chan_part);
        irc_printf(state, "PRIVMSG %s :%s\r\n", nick, line_buffer);
      }

      snprintf(line_buffer, sizeof(line_buffer), "%-*s | %s", max_width, "--- Admin Masks", "Op Masks ---");
      irc_printf(state, "PRIVMSG %s :%s\r\n", nick, line_buffer);

      max_rows = (state->mask_count > state->op_mask_count) ? state->mask_count : state->op_mask_count;
      for (int i = 0; i < max_rows; i++) {
        char admin_part[128] = "";
        char op_part[256] = "";
        if (i < state->mask_count) snprintf(admin_part, sizeof(admin_part), "%s%s", state->auth_masks[i].mask, (state->auth_masks[i].is_managed ? " [M]" : ""));
        if (i < state->op_mask_count) snprintf(op_part, sizeof(op_part), "%.*s (Pass: %.*s)%s", 50, state->op_masks[i].mask, 20, state->op_masks[i].password, (state->op_masks[i].is_managed ? " [M]" : ""));
        snprintf(line_buffer, sizeof(line_buffer), "%-*s | %s", max_width, admin_part, op_part);
        irc_printf(state, "PRIVMSG %s :%s\r\n", nick, line_buffer);
      }

      irc_printf(state, "PRIVMSG %s :--- Trusted Bots (Botpass: %s) ---\r\n", nick, state->bot_comm_pass);
      for (int i = 0; i < state->trusted_bot_count; i++) irc_printf(state, "PRIVMSG %s : - %s\r\n", nick, state->trusted_bots[i]);

      irc_printf(state, "PRIVMSG %s :--- Configured Hubs ---\r\n", nick);
      for(int i=0; i<state->hub_count; i++) irc_printf(state, "PRIVMSG %s : - %s\r\n", nick, state->hub_list[i]);

      size_t footer_len = (size_t)max_width + 40;
      if (footer_len > sizeof(line_buffer) - 1) footer_len = sizeof(line_buffer) - 1;
      memset(line_buffer, '-', footer_len);
      line_buffer[footer_len] = '\0';
      irc_printf(state, "PRIVMSG %s :%s\r\n", nick, line_buffer);
    } else if (strcasecmp(command, "givenick") == 0) {
      irc_printf(state, "PRIVMSG %s :You have about %d seconds to retrieve.\r\n", nick, NICK_TAKE_TIME);
      ...
      if (strlen(arg1) >= MAX_NICK) {
        irc_printf(state, "PRIVMSG %s :Error: Nickname is too long (max %d chars).\r\n", nick, MAX_NICK - 1);
      } else {
        strncpy(state->target_nick, arg1, MAX_NICK - 1);
        state->target_nick[MAX_NICK - 1] = '\0';
        config_write(state, state->startup_password);
        irc_printf(state, "PRIVMSG %s :Target nickname changed to '%s' and saved.\r\n", nick, state->target_nick);
      }
      ...
      if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: setlog <loglevel> :: LOGLEVELS: 0=NONE,15=INFO,63=DEBUG\r\n", nick);
        return;
      }
      bool is_valid_int = true;
      for (int i = 0; arg1[i] != '\0'; i++) if (!isdigit(arg1[i])) { is_valid_int = false; break; }
      if (is_valid_int) {
        int new_level = atoi(arg1);
        state->log_type = (log_type_t)new_level;
        irc_printf(state, "PRIVMSG %s :Log level set to %d.\r\n", nick, new_level);
        config_write(state, state->startup_password);
      } else irc_printf(state, "PRIVMSG %s :Invalid log level. Please provide a valid integer.\r\n", nick);
    } else if (strcasecmp(command, "getlog") == 0) {
      if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: getlog <level> [lines]. Levels are 'msg' 'ctcp' 'info' 'cmd' 'raw' 'debug'. Default: %d. Max: %d.\r\n", nick, DEFAULT_LOG_LINES, MAX_LOG_LINES);
        return;
      }
      int buffer_index = -1;
      if (strcasecmp(arg1, "msg") == 0) buffer_index = 0;
      else if (strcasecmp(arg1, "ctcp") == 0) buffer_index = 1;
      else if (strcasecmp(arg1, "info") == 0) buffer_index = 2;
      else if (strcasecmp(arg1, "cmd") == 0) buffer_index = 3;
      else if (strcasecmp(arg1, "raw") == 0) buffer_index = 4;
      else if (strcasecmp(arg1, "debug") == 0) buffer_index = 5;
      ...
      for (int i = 0; i < LOG_BUFFER_LINES; i++) {
        ...
        if (entry->line[0] != '\0') matches[matches_found++] = entry;
      }

      int lines_to_print = (matches_found < lines_to_show) ? matches_found : lines_to_show;
      int start_index = matches_found - lines_to_print;
      irc_printf(state, "PRIVMSG %s :--- Start of Log (%s) - Showing last %d of %d lines --- \r\n", nick, arg1, lines_to_print, matches_found);
      ...
      if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: +adminmask <nick*!*user@hostmask.com>\r\n", nick);
        return;
      }
      for (int i = 0; i < state->mask_count; i++) {
      ...
      if (state->mask_count < MAX_MASKS) {
        strncpy(state->auth_masks[state->mask_count].mask, arg1, MAX_MASK_LEN - 1);
        state->auth_masks[state->mask_count].is_managed = false;
        state->mask_count++;
        config_write(state, state->startup_password);
      ...
      int found_index = -1;
      for (int i = 0; i < state->mask_count; i++) {
        if (strcasecmp(state->auth_masks[i].mask, arg1) == 0) { found_index = i; break; }
      }
      if (found_index != -1) {
        if (state->auth_masks[found_index].is_managed) {
          irc_printf(state, "PRIVMSG %s :Error: Cannot remove managed mask locally. Use the Hub.\r\n", nick);
          return;
        }
        for (int i = found_index; i < state->mask_count - 1; i++) state->auth_masks[i] = state->auth_masks[i + 1];
        state->mask_count--;
        config_write(state, state->startup_password);
      ...
      if (strlen(arg1) > 0) {
        ...
        config_write(state, state->startup_password);
        irc_printf(state, "PRIVMSG %s :Admin password has been changed and saved.\r\n", nick);
      } else irc_printf(state, "PRIVMSG %s :Error: Password cannot be empty.\r\n", nick);
      ...
      if (state->op_mask_count < MAX_OP_MASKS) {
        strncpy(state->op_masks[state->op_mask_count].mask, arg1, MAX_MASK_LEN - 1);
        strncpy(state->op_masks[state->op_mask_count].password, arg2, MAX_PASS - 1);
        state->op_masks[state->op_mask_count].is_managed = false;
        state->op_mask_count++;
        config_write(state, state->startup_password);
      ...
      int found_index = -1;
      for (int i = 0; i < state->op_mask_count; i++) {
        if (strcasecmp(state->op_masks[i].mask, arg1) == 0) { found_index = i; break; }
      }
      if (found_index != -1) {
        if (state->op_masks[found_index].is_managed) {
          irc_printf(state, "PRIVMSG %s :Error: Cannot remove managed operator locally.\r\n", nick);
          return;
        }
        for (int i = found_index; i < state->op_mask_count - 1; i++) state->op_masks[i] = state->op_masks[i + 1];
        state->op_mask_count--;
        config_write(state, state->startup_password);
      ...
      if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: +server <irc.server.net:6667>\r\n", nick);
        return;
      }
      ...
      } else irc_printf(state, "PRIVMSG %s :Error: Server list is full.\r\n", nick);
    } else if (strcasecmp(command, "-server") == 0) {
      if (!arg1) {
      ...
      int found_index = -1;
      for (int i = 0; i < state->server_count; i++) if (strcasecmp(state->server_list[i], arg1) == 0) { found_index = i; break; }
      if (found_index != -1) {
        free(state->server_list[found_index]);
        for (int i = found_index; i < state->server_count - 1; i++) state->server_list[i] = state->server_list[i + 1];
        state->server_count--;
        state->server_list[state->server_count] = NULL;
        config_write(state, state->startup_password);
        irc_printf(state, "PRIVMSG %s :Removed server '%s' and saved config.\r\n", nick, arg1);
      }
    } else if (strcasecmp(command, "update") == 0) {
      if (arg1) updater_perform_upgrade(state, nick, arg1);
      else updater_check_for_updates(state, nick);
      ...
      int found = -1;
      for (int i=0; i<state->hub_count; i++) if(strcmp(state->hub_list[i], arg1)==0) { found=i; break; }
      if (found != -1) {
        free(state->hub_list[found]);
        for(int i=found; i<state->hub_count-1; i++) state->hub_list[i] = state->hub_list[i+1];
        state->hub_count--;
        config_write(state, state->startup_password);
    ...
    if (!arg1) {
        irc_printf(state, "PRIVMSG %s :Syntax: sethubkey N/TOTAL:data or RESET\r\n", nick);
        ...
        if (slash && colon && slash < colon) {
            ...
            if (part_num >= 1 && part_num <= 16 && total_parts >= 1 && total_parts <= 16) {
                ...
                if ((state->hub_key_parts_received & expected_mask) == expected_mask) {
                    ...
                    if (pem_data && pem_len > 0) {
                        ...
                        bool valid_pem = (memmem(pem_data, pem_len, "-----BEGIN PRIVATE KEY-----", 27) != NULL ||
                                         memmem(pem_data, pem_len, "-----BEGIN RSA PRIVATE KEY-----", 31) != NULL);
                        ...
                        if (valid_pem) {
                            ...
                            config_write(state, state->startup_password);
                            irc_printf(state, "PRIVMSG %s :✓ Complete! Valid PEM key received (%d bytes). Reconnecting...\r\n", 
                                     nick, pem_len);
                            ...
                            irc_printf(state, "PRIVMSG %s :ERROR: Decoded data is not a valid PEM private key. Use RESET.\r\n", nick);
                            free(pem_data);
                        }
                    } else {
                        irc_printf(state, "PRIVMSG %s :ERROR: Invalid base64 encoding. Use RESET and try again.\r\n", nick);
                        if (pem_data) free(pem_data);
                    }
                } else {
                    // Still waiting for more parts
                    int received_count = 0;
                    for (int i = 0; i < total_parts; i++) {
                        if (state->hub_key_parts_received & (1 << i)) received_count++;
                    }
                    irc_printf(state, "PRIVMSG %s :Part %d/%d received (%d/%d complete).\r\n", 
                             nick, part_num, total_parts, received_count, total_parts);
                }
            } else {
                irc_printf(state, "PRIVMSG %s :ERROR: Invalid part number or total. Must be 1-16.\r\n", nick);
            }
        } else {
            ...
            if (pem_data && pem_len > 0) {
                ...
                if (valid_pem) {
                    ...
                    config_write(state, state->startup_password);
                    irc_printf(state, "PRIVMSG %s :✓ Valid PEM key set (%d bytes). Reconnecting...\r\n", 
                             nick, pem_len);
                    ...
                    irc_printf(state, "PRIVMSG %s :ERROR: Not a valid PEM private key.\r\n", nick);
                    free(pem_data);
                }
            } else {
        ...
        if (strlen(sanitized) != 36 || 
            sanitized[8] != '-' || sanitized[13] != '-' || 
            sanitized[18] != '-' || sanitized[23] != '-') {
            irc_printf(state, "PRIVMSG %s :Invalid UUID format. Expected: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx\r\n", nick);
            return;
        }
        ...
        config_write(state, state->startup_password);
        irc_printf(state, "PRIVMSG %s :UUID set to: %s. Reconnecting to hub...\r\n", nick, state->bot_uuid);
      ...
      if (!arg1) {
        irc_printf(state,
                   ...
                   "+hub, -hub, sethubkey, setuuid, saveconf, setlog, getlog, update, help\r\n",
                   nick);
      } else {
        if (strcasecmp(arg1, "die") == 0) {
          ...
          irc_printf(
            ...
            irc_printf(state, "PRIVMSG %s :Syntax: +hub <ip:port> - Add a Hub server.\r\n", nick);
        }
        else if (strcasecmp(arg1, "sethubkey") == 0) {
            irc_printf(state, "PRIVMSG %s :Syntax: sethubkey <key> - Set the Identity Key.\r\n", nick);
        }
        else if (strcasecmp(arg1, "setuuid") == 0) {
            irc_printf(state, "PRIVMSG %s :Syntax: setuuid <uuid> - Set the UUID identifier for HUB connections. Expected: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.\r\n", nick);
        } else {
          irc_printf(state,
    ...
    if (strcasecmp(command, "op") == 0 && arg1) {
      ...
      if (op_arg1) irc_printf(state, "MODE %s +o %s\r\n", op_arg1, nick);
    }
  }
}

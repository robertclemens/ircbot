...
extern int crypto_aes_gcm_encrypt(const unsigned char *plain, int plain_len, const unsigned char *key, unsigned char *output, unsigned char *tag);
extern int crypto_aes_gcm_decrypt(const unsigned char *input, int input_len, const unsigned char *key, unsigned char *output, unsigned char *tag);
...
static int rsa_decrypt_with_bot_privkey(const char *b64_priv_key,
    ...
    if (!b64_priv_key || !enc_data) return -1;
    ...
    if (ctx) {
        if (EVP_PKEY_decrypt_init(ctx) > 0) {
            ...
            size_t out_len;
            if (EVP_PKEY_decrypt(ctx, NULL, &out_len, enc_data, (size_t)enc_len) > 0) {
                if (out_len <= 256) {
                    if (EVP_PKEY_decrypt(ctx, out_plain, &out_len, enc_data, (size_t)enc_len) > 0) {
                        res = (int)out_len;
                    } else {
...
static int rsa_sign_with_bot_privkey(const char *b64_priv_key,
    ...
    if (!b64_priv_key || !data || !sig_out) return -1;
    int pem_len = 0;
    unsigned char *pem_data = base64_decode(b64_priv_key, &pem_len);
...
void hub_client_heartbeat(bot_state_t *state) {
    if (state->hub_count == 0 || !state->hub_connected || !state->hub_authenticated || state->hub_fd == -1) return;
    time_t now = time(NULL);
    if (now - state->last_hub_ping_time < 30) return;
    ...
    int enc_len = crypto_aes_gcm_encrypt(plain, 5, state->hub_session_key, buffer + 4, tag);
    if (enc_len > 0) {
...
void hub_client_sync_hostmask(bot_state_t *state) {
  ...
  if (payload_len < 0 || payload_len >= (int)sizeof(payload)) {
    log_message(L_INFO, state,
                "[WARN] Hostmask payload too large\n");
    return;
  }
  ...
  log_message(L_DEBUG, state,
              "[DEBUG] Syncing hostmask to hub(s): %s\n",
...
void hub_client_generate_config_payload(bot_state_t *state, char *buffer,
  ...
  for (int i = 0; i < state->op_mask_count; i++) {
    ...
    written = snprintf(buffer + offset, max_len - offset,
                      ...
                      op, (long)state->op_masks[i].timestamp);
    if (written < 0 || written >= max_len - offset) break;
    offset += written;
  }
...
void hub_client_push_config(bot_state_t *state) {
  ...
  int cipher_len = crypto_aes_gcm_encrypt(plain, 5 + pay_len,
                                         state->hub_session_key,
                                         cipher + 4, tag);
...
void hub_client_process_config_data(bot_state_t *state, const char *payload) {
  log_message(L_DEBUG, state,
              "[HUB] Processing config data from hub\n");
 ...
 if (key[0]) {
          ...
          if (sscanf(data, "%127[^|]|%7[^|]|%ld", mask, op, &ts) == 3) {
            ...
            if (idx == -1 && is_add) {
              ...
              if (state->mask_count < MAX_MASKS) {
                ...
                if (mask_len >= MAX_MASK_LEN) mask_len = MAX_MASK_LEN - 1;
          ...
          if (sscanf(data, "%127[^|]|%127[^|]|%7[^|]|%ld",
            ...
            if (idx == -1 && is_add) {
              if (state->op_mask_count < MAX_OP_MASKS) {
                ...
                if (pass_len >= MAX_PASS) pass_len = MAX_PASS - 1;
                memcpy(state->op_masks[state->op_mask_count].password, pass, pass_len);
  ...
  if (updates > 0) {
    log_message(L_INFO, state,
                "[HUB] Applied %d config updates from hub\n", updates);
    config_write(state, state->startup_password);
  }
}

void hub_handle_response(bot_state_t *state, int cmd, char *payload, int payload_len) {
    switch(cmd) {
        case CMD_PING:
...
void hub_client_connect(bot_state_t *state) {
    if (state->hub_count == 0 || state->hub_fd != -1 || state->hub_connecting) return;
if (state->bot_uuid[0] == '\0') {
        log_message(L_INFO, state, "[HUB] Cannot connect: UUID not set. Use 'setuuid' command.\n");
        state->last_hub_connect_attempt = time(NULL) + 3600;  // Don't retry for 1 hour
        return;
    }

    // Validate UUID format (36 chars, dashes in right places)
    if (strlen(state->bot_uuid) != 36 ||
        state->bot_uuid[8] != '-' || state->bot_uuid[13] != '-' ||
        state->bot_uuid[18] != '-' || state->bot_uuid[23] != '-') {
        log_message(L_INFO, state, "[HUB] Cannot connect: Invalid UUID format (%s). Use 'setuuid' command.\n",
                   state->bot_uuid);
    ...
    if (state->hub_key[0] == '\0') {
        log_message(L_INFO, state, "[HUB] Cannot connect: Hub key not set. Use 'sethubkey' command.\n");
        state->last_hub_connect_attempt = time(NULL) + 3600;
        return;
    }
    ...
    if (key_len < 1400 || key_len > 2300) {
        log_message(L_INFO, state, "[HUB] Cannot connect: Hub key length suspicious (%zu chars). Expected 1400-2300. Key may be truncated/corrupted. Use 'sethubkey' multipart mode.\n", key_len);
        state->last_hub_connect_attempt = time(NULL) + 3600;
        return;
    }
    ...
    if (state->hub_list[0] == NULL || state->hub_list[0][0] == '\0') {
        log_message(L_INFO, state, "[HUB] Cannot connect: No hubs configured. Use '+hub' command.\n");
        state->last_hub_connect_attempt = time(NULL) + 3600;
        return;
    }
    time_t now = time(NULL);
    if (now - state->last_hub_connect_attempt < HUB_RECONNECT_DELAY) return;
    static volatile int lock = 0;
    if (__sync_lock_test_and_set(&lock, 1)) return;
    state->last_hub_connect_attempt = now;
    state->hub_connecting = true;
    char hub_tmp[256];
    snprintf(hub_tmp, sizeof(hub_tmp), "%s", state->hub_list[rand() % state->hub_count]);
    ...
    *p = '\0';
    ...
    if (send(state->hub_fd, &net_len, 4, 0) == 4 && send(state->hub_fd, state->bot_uuid, uuid_len, 0) == uuid_len) {
...
void hub_client_process(bot_state_t *state) {
    if (state->hub_count == 0 || state->hub_fd == -1) return;
    ...
    while (total_read < packet_len) {
        int r = recv(state->hub_fd, packet_body + total_read, packet_len - total_read, 0);
        if (r <= 0) {
            free(packet_body);
            hub_client_disconnect(state);
            return;
    ...
    if (!state->hub_authenticated) {
        if (auth_state == HUB_AUTH_SENT_UUID) {
            ...
            if (rsa_decrypt_with_bot_privkey(state->hub_key, packet_body, packet_len, challenge) == 32) {
                memcpy(challenge_received, challenge, 32);
                unsigned char signature[512];
                int sig_len = rsa_sign_with_bot_privkey(state->hub_key, challenge, 32, signature);
                if (sig_len > 0) {
                    uint32_t sig_net_len = htonl(sig_len);
                    if (send(state->hub_fd, &sig_net_len, 4, 0) == 4 && send(state->hub_fd, signature, sig_len, 0) == sig_len) {
            ...
            if (rsa_decrypt_with_bot_privkey(state->hub_key, packet_body, packet_len, session_key) == 32) {
...
hub_client_push_config(state);
 ...
 int plain_len = crypto_aes_gcm_decrypt(packet_body, packet_len - GCM_TAG_LEN,
        ...
        if (plain_len > 0) {
            ...
            if (cmd == CMD_PING) {
                ...
                if (now - last_pong_sent >= 5) {
                    ...
                    int pong_enc = crypto_aes_gcm_encrypt(pong_plain, 5,
                                                          state->hub_session_key,
                                                          pong_buf + 4, pong_tag);
                    if (pong_enc > 0) {
